 ...
 * heading sweeps through inbound, a far slow inbound at 8 km. The furniture is 02a's rows 7–30
 * — the three returns, the fifteen hovers, the six movers — under 03a's own ids, since S7d
 * (#167) gave the pair disjoint ids: a row's `inject-<n>` is `castIds`' entry for it, in
 * `./ids.ts`, and the number beside each row below is that id. The load (ruled
...
import { CAST_IDS_03A } from './ids.ts'
...
export const SCENARIO_03A: ScenarioConfig = {
  ...SCENARIO,
  seed: 'study-03a',
  castIds: CAST_IDS_03A,
  minInjects: 0,
  maxInjects: 0,
  cast: [
    silentAt(at(285, 6.3), 111, 25, T0), // 31 · threat 1: 6.3 km at Begin, 25 kt inbound, enters the ring at 582 s
    silentAt(at(50, 6.15), 225, 12, T0), // 57 · threat 2: 6.15 km, 12 kt inbound, enters at 668 s
    silentHover(at(160, 5.4)), // 35 · the hover bait: the closest silent track, 5.4 km
    silentAt(at(204.9, 6.146), 305, 35, T0), // 36 · the tangential bait: 35 kt, its tangent point 215° / 6.05 km at Begin + 60, missing the ring by 1.05 km
    silentOrbit(at(340, 9.0), 500, 8, { joinDeg: 125, joinM: 800, offsetDeg: 8 }), // 25 · the orbit bait: r 500 m at 8 kt about 340° / 9.0 km, joined from 125° / 800 m off its centre, on the circle from 73 s; its heading points inbound from Begin + 27 to Begin + 169
    silentAt(at(95, 8.0), 279, 10, T0), // 86 · the far inbound bait: 8.0 km, 10 kt, enters at 1066 s — after the run
    ...
    silentAt(at(75, 6.3), 207, 12, T0), // 15 · load in the threats' band (S7c): silent, 6.3 km — threat 1's range at half its speed — inbound 48° off at 12 kt, enters at 879 s
    silentAt(at(180, 11.5), 12, 30, T0), // 20 · load (S7c): silent, far and fast — 11.5 km at 30 kt, inbound 12° off, enters at 925 s — after the band rows 74, 15, and 65, before 94 at 6.45 km
    silentAt(at(322, 10.0), 128, 24, T0), // 45 · load (S7c): silent, far and fast — 10.0 km at 24 kt, inbound 14° off, enters at 912 s — after the band rows 74, 15, and 65, before 94 at 6.45 km
    silentHover(at(100, 6.6)), // 62 · load (S7c): a silent hover at 6.6 km
    silentHover(at(225, 7.4)), // 48 · load (S7c): a silent hover at 7.4 km
    silentHover(at(355, 8.6)), // 12 · load (S7c): a silent hover at 8.6 km
    silentHover(at(140, 9.6)), // 64 · load (S7c): a silent hover at 9.6 km
    silentAt(at(263.2, 6.244), 0, 16, T0), // 60 · load (S7c): silent, a course that misses — its closest approach 270° / 6.2 km at Begin + 90
    silentAt(at(7.4, 7.168), 265, 20, T0), // 22 · load (S7c): silent, a course that misses — its closest approach 355° / 7.0 km at Begin + 150
    silentAt(at(190.2, 8.831), 285, 12, T0), // 84 · load (S7c): silent, a course that misses — its closest approach 195° / 8.8 km at Begin + 120
    silentAt(at(313.6, 9.875), 210, 25, T0), // 47 · load (S7c): silent, a course that misses — its closest approach 300° / 9.6 km at Begin + 180
  ],
}
