 ...
 * R1): five more silent steady inbounds at 9–12 km whose entries fall 630 s or more after Begin,
 * and two silent movers on courses that miss, so *silent and pointing at the ring* is a question
 * only the entry time settles — thirteen silent tracks, eight of them steady inbound. A mover is
 * written where it is at Begin; its origin is flown back along its course (`silentAt`).
 */
...
export const SCENARIO_03A: ScenarioConfig = {
  ...
  cast: [
    ...
    silentAt(at(20, 9.5), 205, 14, T0), // 41 · load: silent, steady inbound, enters at 1105 s — after the run
    silentAt(at(125, 11.0), 301, 18, T0), // 42 · load: silent, steady inbound, enters at 1128 s
