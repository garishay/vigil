...
import { SCORING, type Band, type PatternKind, type ScoringConfig } from '../src/config/scoring.ts'
...
export interface InjectRun {
  ...
  last: Band
  /** The named pattern last seen, and how often it changed — the onset itself is one (#155). */
...
export function foldInject(
  ...
  // A kind change is counted whether or not the band moved: the record logs the onset either way.
  const named = kind === run.kind ? run : { ...run, kind, kindChanges: run.kindChanges + 1 }
  if (band === named.last) return named
  const next = { ...named, last: band }
    ...
    for (const seed of seeds) {
      ...
      for (let tSec = 0; tSec <= index.durationS; tSec++) {
        ...
        for (const e of ranked) {
          ...
          runs.set(
            e.track.id,
            foldInject(runs.get(e.track.id), spec, band, tSec, e.rank, e.score.pattern),
          )
        }
      }
...
export function renderBench(result: BenchResult): string {
  ...
  for (const recording of result.recordings) {
    ...
    for (const behavior of BEHAVIORS) {
      ...
      const kinds = runs.map((run) => run.kindChanges)
      lines.push(
        `| ${behavior} | ${runs.length} | ${crossingCells(runs, 'caution')} | ${crossingCells(runs, 'warning')} | ${flaps.reduce((a, b) => a + b, 0)} / ${Math.max(0, ...flaps)} | ${kinds.reduce((a, b) => a + b, 0)} / ${Math.max(0, ...kinds)} |`,
      )
    }
